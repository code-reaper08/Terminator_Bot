...
export default function FunctionTray({ requestsArr }) {
  ...
  const handleSL = async (requester_id, request_id) => {
    ...
    await axios
      .put(`http://localhost:8081/person/updateRequest/${requester_id}`, result)
      .then((res) => {
  ...
  const handleSM = async (requester_id, request_id) => {
    ...
    // await axios.put(
    //   `http://localhost:8081/person/updateRequest/${request_id}`,
    //   request
    // );
    window.location.reload()
  };
          ...
          return (
            <div key={each?.id} className="container">
              <div className="d-flex flex-wrap gap-2">
                <button
                  onClick={() => handleSL(each?.person?.employeeID, each?.resignation?.employeeID)}
                  className="btn btn-primary ternary-bg"
                >
                  Surrender Laptop
                </button>
                <button
                  onClick={() => handleSM(each?.person?.employeeID, each?.resignation?.employeeID)}
                  className="btn btn-primary ternary-bg"
                >
                  Surrender Mobile
                </button>
                <button
                  onClick={() => handleAC(each?.person?.employeeID, each?.resignation?.employeeID)}
                  className="btn btn-primary ternary-bg"
                >
                  Surrender Access Card
                </button>
                <button
                  onClick={() => handleMoney(each?.person?.employeeID, each?.resignation?.employeeID)}
                  className="btn btn-primary ternary-bg"
                >
                  Pay the owed money (balance pay)
                </button>
                <button
                  onClick={() => handleBenifits(each?.person?.employeeID, each?.resignation?.employeeID)}
                  className="btn btn-primary ternary-bg"
                >
